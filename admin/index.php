...
require_once WPSUITE_PATH . 'admin/model.php';
class Admin
{
    ...
    function addMenu()
    {
        ...
        $connect_suffix = add_submenu_page(
            WPSUITE_SLUG,
            __('Connect your Site', 'hub-for-wpsuiteio'),
            __('Connect your Site', 'hub-for-wpsuiteio'),
            'manage_options',
            WPSUITE_SLUG,
        ...
        $diagnostics_suffix = add_submenu_page(
            WPSUITE_SLUG,
            __('Diagnostics', 'hub-for-wpsuiteio'),
            __('Diagnostics', 'hub-for-wpsuiteio'),
            'manage_options',
        ...
        add_action('admin_enqueue_scripts', function ($hook) use ($connect_suffix, $diagnostics_suffix) {
            ...
            $script_asset = array();
            if (file_exists(filename: WPSUITE_PATH . 'wpsuite-admin/dist/index.asset.php')) {
                $script_asset = require_once(WPSUITE_PATH . 'wpsuite-admin/dist/index.asset.php');
            }
            wp_enqueue_script('wpsuite-admin-script', WPSUITE_URL . 'wpsuite-admin/dist/index.js', $script_asset['dependencies'], WPSUITE_VERSION, true);

            if (function_exists('wp_set_script_translations')) {
                wp_set_script_translations('wpsuite-admin-script', 'hub-for-wpsuiteio', WPSUITE_PATH . '/languages');
            }

            wp_enqueue_style('wpsuite-admin-style', WPSUITE_URL . 'wpsuite-admin/dist/index.css', array('wp-components'), WPSUITE_VERSION);

            $view = ($hook === $diagnostics_suffix) ? 'diagnostics' : 'connect';
            $js = 'WpSuite.view = ' . wp_json_encode($view) . ';';
            wp_add_inline_script('wpsuite-main-script', $js, 'after');
        });
    }
            ...
            array(
                ...
                'permission_callback' => function () {
                    if (!current_user_can('manage_options')) {
                        return new WP_Error('rest_forbidden', esc_html__('Forbidden', 'hub-for-wpsuiteio'), array('status' => 403));
                    }
                    return true;
                },
            )
    ...
    public function handleDiagnostics(\WP_REST_Request $request)
    {
        ...
        $main_file = trailingslashit(WPSUITE_PATH) . 'hub-for-wpsuiteio.php';
        ...
        $rest_url = rest_url();
        $r = wp_remote_get($rest_url, array('timeout' => 8));
        if (is_wp_error($r)) {
            $rest_ok = false;
        ...
        $home = home_url('/');
        $lb = wp_remote_get($home, array('timeout' => 8));
        if (is_wp_error($lb)) {
            $loopback_ok = false;
        ...
        $uploads_writable = $wp_filesystem->is_dir($uploads_path) && $wp_filesystem->is_writable($uploads_path);
    ...
    function updateSiteSettings(WP_REST_Request $request)
    {
        ...
        return new WP_REST_Response(array('success' => true, 'message' => __('Site settings updated successfully.', 'hub-for-wpsuiteio')), 200);
    }
